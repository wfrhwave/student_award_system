        ...
        switch(obj.event){
            ...
            case 'update':
                if(data.length === 0){
                    ...
                    var userdata =  data[0];
                    var stu_grades_id = userdata.stu_grades_id;
                    var url = 'student/toeditstudentgrade/'+stu_grades_id
                    $(window).attr('location',url);
                ...
                if(data.length === 0){
                    ...
                    layer.confirm("是否确认删除？",function () {
                        for(var i=0;i<data.length;i++){
                            var stu_grades_id = data[i].stu_grades_id;
                            $.ajax({
                                type:"post",
                                url:'student/deletestudentscore/'+stu_grades_id
                            });
    ...
    table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){
            ...
            layer.confirm('是否确认删除？', function(index){
                ...
                layer.close(index);
                var stu_grades_id = obj.data.stu_grades_id;
            ...
            var stu_grades_id = data.stu_grades_id;
            var url = 'student/toeditstudentclass/'+stu_grades_id
            $(window).attr('location',url);
