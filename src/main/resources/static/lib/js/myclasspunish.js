...
layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'slider'], function(){
    ...
    table.render({
        ...
        ,cols: [ [ //表头
            ...
            ,{field: 'student', title: '姓名', width:'8%',templet:function (student) {
                    return student.student.stu_name;
                }}
            ,{field: 'student', title: '班级', width:'8%',templet:function (student) {
                    return student.student.class_id;
                },sort: true}
            ,{field: 'punish', title: '内容', width:'10%', sort: true}
            ,{field: 'pn_time', title: '时间', width:'10%', sort: true}
    ...
    table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'edit'){
            var reward_id = data.reward_id;
            var url = 'reward/toeditreward/'+reward_id
            $(window).attr('location',url);
            ...
            layer.confirm('是否确认删除？', function(index){
                ...
                layer.close(index);
                var reward_id = obj.data.reward_id;

                $.ajax({
                    type:"post",
                    url:'reward/delete/'+reward_id
